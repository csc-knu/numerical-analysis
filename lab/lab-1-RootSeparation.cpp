...
namespace NumericalAnalysis {
	...
	std::vector<segment> separateRoots(ldouble(*f)(ldouble),
		ldouble left, ldouble right, ldouble precision) {
		...
		std::transform(std::begin(nodesGrid), std::end(nodesGrid),
			std::begin(nodesValues), f);

		std::vector<segment> answer;
		for (size_t i = 0; i + 1 < numNodes; ++i) {
			if (nodesValues[i] * nodesValues[i + 1] < 0) {
