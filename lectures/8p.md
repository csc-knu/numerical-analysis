...
\begin{equation}
	\begin{aligned}
		\Delta(f) & \le \left\| f - \alpha \Phi_0^{(1)} - (1 - \alpha) \Phi_0^{(2)} \right\| = \left\| \alpha \left( f - \Phi_0^{(1)} \right) + (1 - \alpha) \left( f - \Phi_0^{(2)} \right) \right\| \le \newline
		& \le \alpha \left\| f - \Phi_0^{(1)} \right\| + (1 - \alpha) \left\| f - \Phi_0^{(2)} \right\| = \alpha \Delta (f) + (1 - \alpha) \Delta (f) = \Delta(f).
	\end{aligned}
\end{equation}
$$
...
\begin{equation}
	\left\| \alpha \left( f - \Phi_0^{(1)} \right) + (1 - \alpha) \left( f - \Phi_0^{(2)} \right) \right\| = \alpha \left\| f - \Phi_0^{(1)} \right\| + (1 - \alpha) \left\| f - \Phi_0^{(2)} \right\|.
\end{equation}
...
\begin{equation}
	\|f - \lambda g\|^2 = \|f\| - 2 \lambda \langle f, g \rangle + \lambda^2 \|g\|^2 = \|f\|^2 - \lambda \|f\| \cdot \|g\| + \lambda^2 \|g\|^2 = \left( \|f\| - \lambda \|g\| \right)^2.
\end{equation}
...
3. Потрібно наблизити $$f(x) = x^{n + 1}$$, $$x \in [-1,1]$$ багаточленом степеня $$n$$: $$Q_n^0(x)$$. Введемо
	...
	$$
	\begin{equation}
		\Delta(f) = \Inf_{Q_n(x)} \left\| x^{n + 1} - Q_n^0(x) \right\|_C = \Inf_{\overline{P}_{n + 1}} \left\| \overline{P}_{n + 1} - 0 \right\|_C = \left\| \overline{T}_{n + 1}(x) \right\|.
	\end{equation}
	$$
...
\begin{equation}
	\|f - \Phi_2\|^2 = \langle f - \Phi_2, f - \Phi_2 \rangle = \|f - \Phi_0\|^2 - \alpha^2 < \|f - \Phi_0\|^2.
\end{equation}
...
\begin{equation}
	\begin{aligned}
		...
		&= \|f\|^2 - 2 \|\Phi_0\|^2 - 2 \langle \nu, \Phi_0\rangle + \|\Phi_0\| = \|f\|^2 - \|\Phi_0\|^2 = \newline
		&= \Sum_{i = 0}^\infty c_i^2 - \Sum_{i=0}^n c_i^2 = \Sum_{i = n + 1}^\infty c_i^2 \xrightarrow[n \to \infty]{} 0.
	\end{aligned}
\end{equation}
...
> \begin{equation}
> \begin{aligned}
> \Delta^2(f) = \|f - \Phi_0\|^2 = \|f\|^2 - 2 \langle f, \Phi_0 \rangle + \|\Phi_0\|^2 = \|f\|^2 - \|\Phi_0\|^2 = \|f\|^2 - \Sum_{i = 0}^n c_i^2.
> \end{aligned} 
> \end{equation}
...
\begin{equation}
	\label{eq:8.8.8}
	\Phi_1(g) = \Sum_{i = 1}^{n} \Int_{x_{i - 1}}^{x_i} \left(m_{i - 1} \cdot \frac{x_i - x}{h_i} + m_i \cdot \frac{x - x_{i - 1}}{h_i}\right)^2 \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - \mu_i \right)^2 = \inf \Phi_1(u).
\end{equation}
...
\begin{equation}
	\begin{aligned}
		\Phi_1(g) &= \Sum_{i = 1}^{n} \Int_{x_{i - 1}}^{x_i} \left(m_{i - 1} \cdot \frac{x_i - x}{h_i} + m_i \cdot \frac{x - x_{i - 1}}{h_i} \right)^2 \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - \mu_i \right)^2 = \newline
		&= \Sum_{i = 1}^{n} m_i \left(\frac{h_i m_{i - 1}}{6} + \frac{(h_i + h_{i + 1}) m_i}{3} + \frac{h_{i + 1} m_{i + 1}}{6} \right) \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - \mu_i \right)^2 = \newline
		&= \left\langle A \vec m, \vec m \right\rangle + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - \mu_i \right)^2.
	\end{aligned}
\end{equation}
