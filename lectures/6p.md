...
\begin{equation}
    \sign \left( Q_{n - 1}(x_k') \right) = \sign \left( \overline T_n(x_k') - \overline Q_n(x_k') \right) = \sign \left( \overline T_n(x_k') \right) = \alpha \cdot(-1)^k,
\end{equation}
...
\begin{equation}
    H_3^i(x) = y_i + y_i'(x-x_i) + \frac{y_{i-1,i}-y_i'}{h_i} \cdot (x-x_i)^2 + \frac{y_i' - 2 y_{i-1,i}+y_{i-1}'}{h_i^2} \cdot (x-x_i)^2 \cdot (x-x_{i-1})
\end{equation}    
...
\begin{equation}
    f_{\bar x, \hat x, i} \equiv \frac{1}{\bar h_i} \left( \frac{f_{i+1}-f_i}{h_{i+1}} - \frac{f_i - f_{i-1}}{h_i} \right) \approx f^{\prime\prime}(x_i) \approx s^{\prime\prime}(x_i),
\end{equation}
...
Позначимо через $$X_h^k = \{ v_h \in C^0(\Omega): v_h \vert_k, \forall k \in T_k\}$$ &mdash; простір інтерполянтів при розбитті області на трикутники, а $$Y_h^k = \{v_h \in C^0(\Omega): v_h \vert_k \in \Theta_k, \forall k \in T_h\}$$ &mdash; при розбитті на прямокутники.
...
$$
\begin{equation}
    \left\|v\right\|^2_m = \Sum_{k = 0}^{m} \left\|v_k\right\|^2,
\end{equation}
$$
