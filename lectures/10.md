...
\begin{equation}
	\begin{aligned}
		...
		&\quad \ldots \le (1 + \tau L)^n |z_0| + \Sum_{j = 0}^{n} \tau (1 + \tau L)^{n - j} |\psi_j| \le \newline
...
І, нарешті, $$R'''(0) = \frac{3}{2} (f_{tt,0} + 2 f_{tu, 0} + f_{uu} \cdot f_0^2 \right) - u'''(t_0) \ne 0$$. Таким чином похибка на одному кроці має порядок $$p = 2$$.
...
\begin{equation}
	\label{eq:10.2.10}
	y_{n + 1} = y_n + \tau f \left( t_{n + 1/2}, y_{n + 1/2})
\end{equation}
