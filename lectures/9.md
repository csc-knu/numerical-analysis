...
\begin{equation}
	I_h(f) = \Sum_{i = 1}^N \frac{h_i}{2} \left( f(x_{i-1}) + f(x_i)),
\end{equation}
...
Всі значення крім останнього $$I_{h/8}^{(3)}$$ можна оцінити за принципом Рунге (див.&nbsp;формулу \eqref{eq:9.5.2}). Використання формули \eqref{eq:9.5.5} для обчислення $$I_{h/2^k}^{(0)}$$ та лінійні комбінації \eqref{eq:9.5.2} дають простий та економічний алгоритм обчислення $$I$$. Початкове значення $$h$$ можна брати рівним $$b - a$$, або $$\frac{b - a}{n}$$, де $$n$$ ціле.
