...
> \label{eq:9.4.2}
> \begin{aligned}
> R_2(f) &= \frac{1}{24} \Int_a^b (x - a) \left( x - \frac{a + b}{2} \right)^2 (x - b) f^{(4)}(\xi) \diff x = \newline
> &= \frac{f^{(4)}(\xi)}{2880} \cdot (b - a)^5,
> \end{aligned}
> \end{equation}
>
> та вірна оцінка:
...
\begin{equation}
	...
	\begin{aligned}
		...
		&= \frac{h}{6} \left( f(x_0) + 4 f(x_{1/2}) + 2 f(x_1) + \ldots+ 2 f(x_{N - 1}) + 4 f(x_{N-1/2}) + f(x_N) \right).
	\end{aligned}
\end{equation}
