...
2. _Адитивний спосіб_. Представимо підінтегральну функцію у вигляді $$F(x) = f(x)+ \psi(x)$$, причому $$\psi(x)$$ &mdash; особлива, $$f(x)$$ &mdash; гладка. Розбиваємо інтеграл на два: $$I_ = I_1 + I_2$$. 
	...
	2. $$I_2 = \Int_0^{\pi/2} \ln(x) \diff x = \left. (x \ln(x) - x) \right\vert_0^{\pi/2} = \frac{\pi}{2} \left( \ln \left( \frac{\pi}{2} \right) - 1 \right)$$.
...
1. _Заміни_. Нехай $$а > 0$$. Зробимо заміну $$t = \frac{x - a}{x}$$, $$x = \frac{a}{1 - t}$$. Тоді
	...
	Якщо $$а = 0$$, то робимо заміну $$t = e^{-x}$$, $$x = - \ln(t)$$, тоді
	...
	Якщо $$а < 0$$ (не можна зробити заміну $$t = \frac{x - a}{x}$$, тому що виникає особливість в точці $$x = 0$$), розбиваємо інтеграл на два:
