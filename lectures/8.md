...
\begin{equation}
	...
	\left\\{
		\begin{aligned}
			...
			a_k &= \langle f, \varphi_{2 k - 1} = \frac{1}{\pi} \Int_0^{2 \pi} f(x) \cos (k x) \diff x, \newline
			b_k &= \langle f, \varphi_{2 k} = \frac{1}{\pi} \Int_0^{2 \pi} f(x) \sin (k x) \diff x.
		\end{aligned}
	\right.
\end{equation}
...
Це _формули Бесселя_. В формулі \eqref{eq:8.6.1}: $$\Phi(x) \equiv T_n(x)$$ (тобто багаточлен той же), але коефіцієнти визначаємо за формулою \eqref{eq:8.6.3}.
...
Для визначення коефіцієнтів $$a_i$$, $$b_i$$ за формулою Бесселя \eqref{eq:8.6.3}  необхідна кількість операцій $$Q = O(N^2)$$. Існують алгоритми, які дозволяють обчислити за $$Q = O(N \cdot \log (N))$$ операцій. Це так званий алгоритм швидкого перетворення Фур'є. Якщо в \eqref{eq:8.6.3} існує група доданків, які рівні між собою, тобто число $$N$$ можна представити як $$N = p_1 p_2$$, то можна так вибрати сітку, що $$Q = O(N \cdot \max(p_1, p_2))$$. Якщо ж $$N = n^m$$, то $$Q = O(N m) = O(N \log_2(N))$$.
