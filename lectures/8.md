...
3. $$H$$ гільбертів простір з ваговим множником $$\rho(x) = (1 - x)^\alpha (1 + x)^\beta$$. Система $$\varphi_i(x) = P_n^{(\alpha, \beta)}(x)$$ &mdash; _багаточленів Якобі_, $$\alpha, \beta > -1$$ ($$\alpha$$, $$\beta$$ &mdash; числові параметри) ортогональна в сенсі скалярного добутку
	...
	а $$\Gamma(z + 1) = z \Gamma(z)$$, $$\Gamma(n + 1) = n!$$.
...
7. Якщо потрібно апроксимувати табличну функцію, то $$H = \ell_2$$, $$x_i = i$$, $$i = \overline{0, N}$$,
	...
	і за систему ортогональних функцій вибираємо наступну систему багаточленів: $$\varphi_k(x) = p_k^{(N)}(x)$$, $$k = \ovelrine{0,m}$$ ($$m \le N$$) &mdash; систему _багаточленів Чебишова дискретного аргументу_, які задається формулою
	...
	\begin{equation}
		\frac{(m + 1) (N - m)}{2 (2 m + 1)} p_{m + 1}^{(N)} = \left( \frac{N}{2} - x \right) p_m^{(N)} - \frac{m (N + m + 1)}{2 (2 m + 1)} p_{m - 1}^{(N)},
	\end{equation}

	з початковими значеннями $$p_0^{(N)} = 1$$, $$p_{-1}^{(N)} = 0$$.
	
	Наприклад $$p_1^{(N)} = 1 - 2 x / N$$, $$p_2^{(N)} = 1 - 6 x / N + 6 x^2 / N (N - 1)$$.
