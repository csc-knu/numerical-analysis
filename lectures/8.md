...
\begin{equation}
	...
	\left\\{
		\begin{aligned}
			& \frac{h_i m_{i - 1}}{6} + \frac{(h_i + h_{i + 1} m_i)}{3} + \frac{h_{i + 1} m_{i + 1}}{6} = \frac{f_{i + 1} - f_i}{h_{i + 1}} - \frac{f_i - f_{i - 1}}{h_i}, \quad i = \overline{1, n-  1} \newline
			& m_0 = m_n = 0.
		\end{aligned}
	\right.
\end{equation}
...
\begin{equation}
	A = \begin{pmatrix}
		...
		\frac{h_2}{6} & \frac{h_2 + h_3}{3} & \ddots & \ddots & \vdots \newline
		0 & \ddots & \ddots & \ddots & 0 \newline
		\vdots & \ddots & \ddots & \ddots & \ddots \newline
		0 & \ldtos & 0 & \ddots & \ddots
	\end{pmatrix},
\end{equation}
...
\begin{equation}
	\Int_a^b (s''(x))^2 \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - s(x_i) \right)^2 \ge \Int_a^b (g''(x))^2 \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - g(x_i) \right)^2
\end{equation}
...
Підставимо \eqref{eq:8.8.1} та \eqref{eq:8.8.6} в $$\Phi_1(g)$$:
...
\begin{equation}
	\Phi_1(u) + \Int_a^b (u''(x))^2 \diff x + \Sum_{i = 0}^{n} \rho_i \left( \tilde f_i - u(x_i) \right)^2.
\end{equation}
...
\begin{equation}
	\label{eq:8.8.9}
	2 \left( H^\intercal \vec m)\void_i + 2 \rho_i (\mu_i - f_i) = 0,
\end{equation}
...
\begin{equation}
	g(x) = m_{i - 1} \cdot \frac{(x_i - x)^3}{6 h_i} + m_i \cdot \frac{(x - x_{i - 1})^3}{6 h_i} + \left( \mu_{i - 1} - \frac{m_{i - 1} \cdot h_i^2}{6} \right) \cdot \frac{x_i - x}{h_i} + \left( \mu_i - \frac{m_i \cdot h_i^2}{6}\right) \cdot \frac{x - x_{i - 1}}{h_i}, 
\end{equation}
