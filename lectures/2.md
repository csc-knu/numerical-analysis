...
> **Зауваження 2**: Метод Ньютона можна застосовувати і для обчислення комплексного кореня $$z_{k + 1} = z_k - \frac{f(z_k)}{f'(z_k)}$$. В теоремі про збіжність $$q = \frac{\vert x_0 - \bar x \vert M_2}{2 m_1}$$, де $$m_1 = \Min_{U_r} \vert f'(z) \vert$$, $$M_2 = \Max_{U_r} f''(z) \vert$$. Тут $$\vert z \vert$$ &mdash; модуль комплексного числа.
...
- збіжність залежить від початкового наближення $$x_0$$, оскільки від нього залежить умова збіжності $$q = \frac{M_2 \vert x_0 - \bar x \vert}{2 m_1} < 1$$;
