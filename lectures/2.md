...
де $$k = 0, 1, 2, 3 \ldtos$$, а $$x_0$$ &mdash; задано. Метод можна записати у вигляді $$\frac{x_{k + 1} - x_k}{\tau} = f(x_k)$$, $$k = 0, 1, \ldots$$. Оскільки $$\varphi'(x) = 1 + \tau \cdot f'(x)$$, то метод збігається при умові
...
$$
z_{k + 1} = z_k + \tau \cdpt f(x + z_k).
$$

В припущені $$f(x) \in C^1([a,b])$$ з теореми про середнє маємо

$$
f (\overline{x} + z_k) = f(\ovelrine{x}) + z_k \cdot f'(\overline{x} + \theta \cdot z_k) = z_k \cdot  f'(\overline{x} + \theta \cdot z_k) = z_k \cdot f'(\xi_k),
$$
