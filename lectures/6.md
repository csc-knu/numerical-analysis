...
\begin{equation}
	g(t) = f(t) - L_n(t) - K \cdot \omega_n(t), \quad t \in [a,b], \quad g(x_i) = 0, \quad i = \overline{0,n}.
\end{equation}

Знайдемо таке $$K$$, щоб $$g(x) = 0$$, в деякій точці $$x \in [a,b]$$, $$x \ne x_i$$, $$i = \overline{0,n}$$. Легко бачити, що 

\begin{equation}
	K = \dfrac{f(x) - L_n(x)}{\omega_n(x)}.
\end{equation}
...
Коефіцієнт при старшому члені багаточлена дорівнює $$2^{n-1}4$. 
...
\begin{equation}
	\text{sgn} \left( Q_{n - 1}(x_k') \right) = \text{sgn} \left( \overline T_n(x_k') - \overline Q_n(x_k') \right) = \text{sgn} \left( \overline T_n(x_k') \right) = \alpha \cdot(-1)^k, \quad \alpha = \pm 1.
\end{equation}
...
\begin{equation}
	T_n^{[a,b]}(t) = \overline T_n \left( \frac{2 x}{b - a} - \frac{b + a}{b - a} \right) = 2^{1 - n} \cos \left(n \arccos \left( \frac{2 x - (b + a)}{b - a} \right) \right).
\end{equation}
...
\begin{equation}
	x_k = \frac{a + b}{2} - \frac{b - a}{2} \cdot t_k, \quad t_k = \cos \left( \frac{(2 k + 1) \pi}{2 n} \right), \quad k = \overline{0,n-1},
\end{equation}
...
\begin{equation}
	x_k = \frac{a + b}{2} - \frac{b - a}{2} \cdot t_k, \quad t_k = \cos \left( \frac{(2 k + 1) \pi}{2 (n + 1)} \right), \quad k = \overline{0,n}.
\end{equation}
...
\begin{equation} 
	\label{eq:6.4.6}
	|r_n(x)| = |f(x) - L_n(x)| \le \frac{M_{n + 1}}{(n + 1)!} \cdot \dfrac{(b - a)^{n + 1}}{2^{2 n + 1}}.
\end{equation} 
