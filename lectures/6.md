...
\begin{equation}
	\begin{aligned}
		...
		& \quad \ldots + f \left( x_{0, 0}^\varepsilon; \ldots; x_{n, k_n - 1}^\varepsilon \right) \left( x - x_{0, 0}^\varepsilon \right) \ldots \left( x - x_{n, k_n - 1}^\varepsilon \right).
	\end{aligned}
\end{equation}
...
Введемо норму $$\|f(x) - L_n\|\void_{C([a,b])} = \Max_{x \in [a,b]} \vert f(x) - L_n(x) \vert$$. Тоді для довільної $$f(x) \in C^{(n + 1)}([a,b])$$ справджується оцінка
...
де $$M_{n + 1} = \Max_{x \in [a,b]} \vert f^{(n + 1)}(x) \vert$$, $$\omega_n(x) = \Prod_{i = 0}^n (x - x_i)$$. А яка оцінка буде для довільної неперервної функції?

> **Означення**: Кажуть, що інтерполяційний процес для функції $$f(x)$$ _збігається_ в точці $$x \in [a,b]$$, якщо
>
> \begin{equation}
> \Lim_{n\to\infty} L_n(x) = f(x), \quad \forall \{x_i\}^n_{i = 1}: h = \Max_{i = \overline{1,n}} h_i \to 0, \quad h_i = x_i - x_{i-1}.
> \end{equation}
...
> **Теорема**: Стала Лебега $$\|P_n\| = \Max_j \Sum_{j = 0}^n \left\vert \varphi_j^{(n)}(x) \right\vert$$, де $$\varphi_j^{(n)}(x) = \frac{\omega_n(x)}{(x-x_j) \cdot \omega_n'(x_i)}$$.
...
> \end{equation}
>
> де $$E_n(f) = \Inf_{Q_n(x)} \|f(x) - Q_n(x)\|\void_{C([a,b])}$$ &mdash; відхилення багаточлена $$n$$-го степеня найкращого рівномірного наближення від $$f(x)$$.

> **Теорема**: Нехай $$P_n^E$$ &mdash; оператор інтерполяції на рівномірній сітці, $$P_n^\intercal$$ &mdash; оператор інтерполяції на чебишовській сітці. Тоді на $$[-1,1]$$ маємо наближені оцінки:
>
> \begin{equation}
> \|P_n^E\|\approx C_1 \cdot 2^n, \quad \|P_n^\intercal\| \approx C_2 \cdot \ln(n).
> \end{equation}
...
\begin{equation}
    \label{eq:6.21}
    \Phi_1(x) = L_1^i(x) = f(x_{i-1})\cdot \frac{x-x_{i-1}}{x_i-x_{i-1}} + f(x_i) \cdot \frac{x_i-x}{x_i-x_{i-1}}, \quad x \in [x_{i-1}, x_i].
\end{equation}
...
\end{equation}
З умов інтерполювання маємо \[ \Phi_1(x_j) = \Sum_{i=0}^n f(x_i) \cdot \varphi_i(x_j) = f(x_j). \] Звідси \[ \varphi_i(x_j) = \delta_{i,j} = \begin{cases} 1, & i = j \\ 0, & i \ne j \end{cases}. \] Значить \[ \varphi_i(x) = \begin{cases} 0, & a \le x \le x_{i-1} \\ \frac{x-x_{i-1}}{x_i-x_{i-1}}, & x_{i-1} \le x \le x_i \\ \frac{x_{i+1}-x}{x_{i+1}-x_i}, & x_i \le x \le x_{i+1} \\ 0, & x_{i+1} \le x \le b \end{cases} \]
\begin{theorem}
    Для довільної $f(X) \in C^{(2)}([a,b])$ справедлива оцінка
    \begin{equation}
        \label{eq:6.23}
        \|f(x)-\Phi_1(x)\|_{C([a,b])} \le \frac{M_2}{8}\cdot|h|^2,
    \end{equation}
...
\begin{proof}
    ...
    \[ z(x) = f(x) - \Phi_1(x) = f(x) - L_1^i(x) = \frac{f''(\xi_i)}{2!} \cdot(x-x_{i-1})\cdot(x-x_i). \]
    Звідси 
    \begin{equation}
        \label{eq:6.24}
        |f(x) - \Phi_1(x)| \le \frac{M_2^i}{2} \cdot |(x-x_{i-1}) \cdot (x - x_i)| \le \frac{M_2^i \cdot h_i^2}{8},
    \end{equation}
    ...
    \begin{equation}
        \label{eq:6.25}
        \Max_{i=\overline{1,n}} \Max_{x\in[x_{i-1},x_i]} |z(x)| \le \frac18 h^2 M_2,
    \end{equation}
...
\end{problem}
Отже маємо збіжність процесу інтерполювання за допомогою кусково-лінійної функції \[\left\|f(x)-\Phi_1^{(n)}(x)\right\|_{C([a,b])}\xrightarrow[h\to 0, n \to \infty]{}0,\] $\left\{\Phi_1^{(n)}(x)\right\} \implies f(x)$. \\
...
\begin{multline*}
    ...
    + \frac{y_i' - 2 y_{i-1,i}+y_{i-1}'}{h_i^2} \cdot (x-x_i)^2 \cdot (x-x_{i-1})
\end{multline*}
...
Ці функції кусково-кубічні, тобто $\varphi_i^k(x)\in \pi_3$, $x\in[x_{i-1},x_{i+1}]$, $k=0,1$ ($\pi_3$ -- множина багаточленів третього степеня), на всіх інших проміжка вони нульові. Нехай $h_i \equiv h$, і позначимо $s = \frac{x-x_i}{h}$, $x\in[x_{i-1},x_i] \implies s \in [-1,0]$. 
\begin{enumerate}
