...
де $$M_{n + 1} = \Max_{x \in [a,b]} \left\vert f^{(n + 1)}(x) \right\vert$$, $$\omega_n(x) = \Prod_{i = 0}^n (x - x_i)$$. А яка оцінка буде для довільної неперервної функції?
...
> \end{equation}
> 
> де $$\Phi_1(x)$$ &mdash; кусково-лінійна функція, побудована по значеннях $$f(x_i)$$, $$i = \overline{0, n}$$, $$\vert h \vett = \Max_i h_i$$, $$h_i = x_i - x_{i-1}$$.
...
де $$M_2^i = \Max_{x \in [x_{i - 1}, x_i]} \vert f''(x) \vert$$. Остання оцінка отримана з нерівності
...
де $$M_2 = \Max_{x\in[a,b]} |f''(x)|$$, $$h_i = \Max_i h_i$$, що доводить \eqref{eq:6.9.4}. $$\square$$
...
1. $$H_0 = L_2([a,b])$$ &mdash; гільбертів простір, в якому скалярний добуток визначається так: $$\langle u, v \rangle = \Int_a^b (u(x) \cdot v(x)) \diff x$$, а норма $$\|u\|\void_0 = \sqrt{\langle u,u \rangle}$$.
...
> **Теорема**: Нехай $$f(x) \in H_2 = W_2^2([a,b])$$. Тоді $$\left\|f^{(k)} - \Phi_1^{(k)} \right\|\void_0 \le \vert h \vert^{2-k} \cdot \|f\|\void_2$$, $$k=1,2$$. 
