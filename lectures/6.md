...
\begin{equation}
	\begin{aligned}
    	...
    	& \quad - \Sum_{i = 0}^{j - 1} \frac{f(x_i)}{(x_i - x_0) \ldots (x_i - x_{i-1}) (x_i - x_{i + 1}) \ldots (x_i - x_{j - 1}) (x_j-x_i)} = \newline
    	& = \frac{f(x_j)}{(x_j-x_0) \ldots (x_j - x_{j-1})} + \newline
...
\begin{equation}
	\begin{aligned}
		L_n(x) &= L_n(x_0 + t h) = \newline
		&= f_0 + t \Delta f_0 + \ldots + \frac{t\cdot(t-1)\cdot(t-n+1)}{n!}\cdot \Delta^n f_0,
	\end{aligned}
\end{equation}
...
\[f\left(x_{i,0}^\epsilon;\ldots;x_{i,k_i-1}^\epsilon\right)=f(x_i;\ldots;x_i)=\frac{f^{(k_i)}(x_i)}{k_i!}.\]
