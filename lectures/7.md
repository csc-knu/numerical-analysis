...
При $$n \to \infty$$ маємо $$\left\| f(x) - \widetilde{f}(x)\right\|_{C([a,b])} = \frac{1}{n} \to 0$$, звідси $$\widetilde{f}(x) \xrightarrow[n \to \infty]{} f(x)$$. Таким чином це малі збурення. Маємо $$\widetild'(x) = f'(x) + \frac{\omega}{n} \cos(\omega x)$$. Нехай $$\omega = n^2$$, тоді 
...
\begin{equation}
	f_i' = \frac{\widetile{f}_i - \widetilde{f}_{i - 1}}{h} = f_i' - \frac{f_i - f_{i - 1}}{h} - \frac{\delta_i - \delta_{i - 1}}{h},
\end{equation}
