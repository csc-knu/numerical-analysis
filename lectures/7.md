...
При $$n \to \infty$$ маємо $$\left\| f(x) - \widetilde{f}(x)\right\|_{C([a,b])} = \frac{1}{n} \to 0$$, звідси $$\widetilde{f}(x) \xrightarrow[n \to \infty]{} f(x)$$. Таким чином це малі збурення. Маємо $$\widetilde{f}'(x) = f'(x) + \frac{\omega}{n} \cdot \cos(\omega x)$$. Нехай $$\omega = n^2$$, тоді 

\begin{equation}
	\left\| f(x) - \widetilde{f}(x)\right\|_{C([a,b])} = \frac{|\omega|}{n} = n \xrightarrow[n \to \infty]{} \infty.
\end{equation}
...
Нехай $$\widetilde{f}_i = f_i + \delta_i$$, $$f_i = f(x_i)$$, $$i = \overline{0, n}$$, $$\vert \delta_i \vert \le \delta$$. Розглянемо вплив похибок $$\delta_i$$ на конкретних формулах чисельного диференціювання.
...
$$
\begin{equation}
	f_i' = \frac{\widetilde{f}_i - \widetilde{f}_{i - 1}}{h} = f_i' - \frac{f_i - f_{i - 1}}{h} - \frac{\delta_i - \delta_{i - 1}}{h},
\end{equation}
$$
...
\begin{equation}
	\label{eq:7.2.1}
	\left| f_i' - \frac{\widetilde{f}_i - \widetilde{f}_{i - 1}}{h} \right| \le \left| f_i' - \frac{f_i - f_{i - 1}}{h} \right| + \left| \frac{\delta_i - \delta_{i - 1}}{h} \right| \le \frac{M_2 h}{2} + \frac{2 \delta}{h} \xrightarrow[h \to 0]{} \infty
\end{equation}
$$
...
звідки $$h_0 = 2 \sqrt{\frac{\delta}{M_2}}$$. При такому значенні $$h$$ оцінка похибки \eqref{eq:7.2.1} така:
...
$$
\begin{equation}
	\left| f_i' - \frac{\widetilde{f}_{i + 1} - \widetilde{f}_{i - 1}}{2 h} \right| \le \left| f_i' - \frac{f_{i + 1} - f_{i - 1}}{2 h} \right| + \left|\frac{\delta_{i + 1} - \delta_{i - 1}}{2 h} \right| \le \frac{M_3 h^2}{6} + \frac{\delta}{h} = \varphi(h).
\end{equation}
$$
