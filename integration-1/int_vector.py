...
def integrate_simpson(f: Callable[[float], float], a: float or None=None, 
	b: float or None=None, h: float or None=None, n: int or None=None) -> float:
	if a is None:
		a = b - h * n
	if b is None:
	...
	if n is None:
		n = (b - a) / h
	return (b - a) / (6 * n) * (f(a) + 4 * sum(f(a + h * k + h / 2) for k in range(n)) + \
		sum(f(a + k * h) for k in range(1, n)) + f(b))
