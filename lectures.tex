...
Повернемося до формули \eqref{eq:8}. Виходячи з останніх зауважень з'ясуємо який же порядок по $h$ матиме $r_n^{(k)}$. Зрозуміло що найнижчий (тобто найгірший) порядок буде у доданку суми де $j = 0$, а саме:
\begin{equation}
	\label{eq:10}
